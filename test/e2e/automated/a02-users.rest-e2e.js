...
describe('#Users', () => {
  before(async () => {
    ...
    context.email2 = testUser.user.email
  ...
  describe('#GET /users', () => {
    ...
    //     const { token } = context
    //
    //     console.log('uut.useCases.user: ', uut.useCases.user)
    //
    //     // Force an error
    //     sandbox
  ...
  describe('PUT /users/:id', () => {
    ...
    it('should throw 401 if non-admin updating other user', async () => {
      ...
      try {
        const options = {
          method: 'PUT',
          url: `${LOCALHOST}/users/test1@test.com`,
          headers: {
            Accept: 'application/json',
            Authorization: `Bearer ${token}`
          }
    ...
    it('should not be able to update user type', async () => {
      try {
        const options = {
          method: 'PUT',
          url: `${LOCALHOST}/users/${context.user.email}`,
          headers: {
    ...
    it('should not be able to update other user when not admin', async () => {
      try {
        const options = {
          method: 'PUT',
          url: `${LOCALHOST}/users/${context.user2.email}`,
          headers: {
            Authorization: `Bearer ${context.token}`
          },
          data: {
  ...
  describe('DELETE /users/:id', () => {
    ...
    it('should not be able to delete other users unless admin', async () => {
      try {
        const options = {
          method: 'DELETE',
          url: `${LOCALHOST}/users/${context.user2.email.toString()}`,
          headers: {
            Authorization: `Bearer ${context.token}`
          }
        }
